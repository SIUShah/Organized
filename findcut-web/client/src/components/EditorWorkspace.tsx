      ...
      <div className="grid gap-4 lg:grid-cols-[220px_minmax(0,1fr)_240px]">
        ...
        <div className="space-y-3">
          <div className="relative aspect-video overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-cyan-300/20 via-slate-900 to-indigo-400/20">
            {selected?.url && selected.kind === "video" ? <video ref={videoRef} src={selected.url} className="h-full w-full object-contain" controls={false} /> : <div className="absolute inset-0 grid place-items-center text-center text-sm text-slate-400"><div><MonitorIcon /><div className="mt-2">Preview engine ready</div><div className="text-xs text-slate-600">Import a video to preview it here</div></div></div>}
